...
var showRealData = ZSB["能源客户视角"]["变压器"];
$(function() {
    Flush();
    $.ajax({
            ...
            for (let j = 0; j < table_data['data'][index]['table'][i]['data'].length; j++) {
                ...
                for (let k = 0; k < column; k++) {
                    if (k % 2 == 0) {
                        ...
                        if (k == 1 && table_data['data'][index]['table'][i]['data'][j][column - 5]['value'] >= '40℃') {
                            tabvar += "<td class='redValueCell unSelectedCell' style='width:" + table_data['data'][index]['table'][i]['rowHeader'][k]['width'] + "%'>" + table_data['data'][index]['table'][i]['data'][j][k]['value'] + "</td>";
