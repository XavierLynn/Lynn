...
var showRealData = ZSB["能源客户视角"]["设备运行状况分析"];
$(function() {
    Flush();
    $.ajax({
        ...
        for (let i = 0; i < table_data['data'][index]['table'].length; i++) {
            ...
            for (let j = 0; j < table_data['data'][index]['table'][i]['data'].length; j++) {
                ...
                for (let x = 0; x < units.length; x++) {
                    if (currentValue.search(units[x]) != -1) {
                        tabvar += "<td>" + currentValue.split(units[x]).join("<span style = 'font-size:70px; font-family: 黑体'>" + units[x] + "</span>") + "</td >";
                    }
                }
            }
            tabvar += "</tr>";
