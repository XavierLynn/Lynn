...
var showRealData = ZSB["政府视角"]["企业异常用能事件"];
$(function() {
    Flush();
    $.ajax({
        url: "http://localhost:2277/展示版/ZFSJ.asmx/GetQYYCYNSJ",
        type: "POST",
        async: true,
        data: "enteId=94BFAE5209AA44A1969C3AFA13870517&startDate=2018-01&endDate=2019-12",
        error: function() {
            Flush();
        ...
        for (let i = 0; i < table_data['data'][index]['table'].length; i++) {
            ...
            for (let j = 0; j < table_data['data'][index]['table'][i]['data'].length; j++) {
                ...
                for (let k = 0; k < column - 1; k++) {
                    if (k == column - 2) {
                        if (table_data['data'][index]['table'][i]['data'][j][column - 1]['value'] == 'false')
                            tabvar += "<td class='selectedFalseCell'>" + table_data['data'][index]['table'][i]['data'][j][k]['value'] + "</td>";
                        else
