        ...
        "table": [{
            ...
            "data": [
                [{
                    ...
                    {
                        "value": "2019.09.21 18:00:00"
                    },
                    {
                        "value": "2019.09.21 20:00:00"
                    },
                    {
                        "value": "100MW"
                    },
...
var showRealData = ZSB["能源企业视角"]["削峰填谷事件回顾"];
$(function() {
    Flush();
    $.ajax({
...
function Flush(){
    ...
    $('.liLink').click(function() {
            ...
            for (let j = 0; j < table_data['data'][index]['table'][i]['data'].length; j++) {
                ...
                for (let k = 0; k < column; k++) {
                    ...
                    if (table_data['data'][index]['table'][i]['data'][j][column - 4]['value'] == '执行中')
                        tabvar += "<td class='selectedCell " + font + "' style='width:" + table_data['data'][index]['table'][i]['rowHeader'][k]['width'] + "%'>" + table_data['data'][index]['table'][i]['data'][j][k]['value'] + "</td>";
                    else
                        tabvar += "<td class='unSelectedCell " + font + "' style='width:" + table_data['data'][index]['table'][i]['rowHeader'][k]['width'] + "%'>" + table_data['data'][index]['table'][i]['data'][j][k]['value'] + "</td>";
                }
                tabvar += "</tr>"
            }
            tabvar += "</table>";
        ...
        for (let i = 0; i < $('.table-body').find('tbody').length; i++) {
            if ($(".table-body").find("tbody").eq(i).find("tr").length >= 11) {
                $(".table-body").css('height', "1330px");
            }
        }
        $('.table-body tr').click(function() {
            $('.table-body tr').removeClass('selectedTr');
            $(this).addClass('selectedTr');
            ue4("ListItemClick", $(this).children().first().text());
        });
    })
    $('.liLink')[0].click();
    $('.nav a')[0].click();  //延迟刷新
    ...
    $('.unSelectedCell').css("color", mainColor);
