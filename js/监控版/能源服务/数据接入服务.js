var showRealData = JKB["能源服务"]["数据接入服务"]
    ...
    $('.liLink').click(function() {
        ...
        for (let i = 0; i < table_data['data'][index]['table'].length; i++) {
            ...
            for (let j = 0; j < table_data['data'][index]['table'][i]['rowHeader'].length; j++) {
                ...
                tabvar += "<td>" + currentHeader['headerName']  + "<span style = 'word-spacing:250px'>" + ' ' + "</span>" + "<span style = 'font-size:88px;padding-top:10px 5px 5px 20px;text-align:center;font-family:庞门正道标题体'>" + currentValue + "</span>" + "</td>";
            }
            tabvar += "</tr>";
            tabvar += "</table>";
            tabvar += "</div>";
